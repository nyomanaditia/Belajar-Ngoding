...
function economyChangeSummary(tradeActivity) {
  ...
  for (let i = 0; i < tradeActivity.length; i++) {
    for (let j = 0; j < tradeActivity[i].length; j++) {
      ...
      const regexPersetase = /[0-9]+/g;
      const regexOperasi = /\+|-/g;
      const operasi = tradeActivity[i][j].match(regexOperasi).toString();
      const persentase = tradeActivity[i][j].match(regexPersetase).toString();

      //menghitung persentase
      const plusMinusDeposit = deposit * (persentase / 100);

      //menambahkan atau mengurangkan deposit;
      if (operasi == "+") {
        deposit += plusMinusDeposit;
