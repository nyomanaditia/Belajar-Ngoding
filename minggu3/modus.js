...
function cariModus(arr) {
  ...
  // Modus dengan array terpanjang
  let panjangModusTerbesar = 0;
  for(let i = 0; i < result.length; i++) {
    if(result[i].length > panjangModusTerbesar) {
      panjangModusTerbesar = result[i].length;
    }
  }

  // Mereturn modus
  for(let i = 0; i < result.length; i++) {
    if(panjangModusTerbesar< 2) {
      return -1;
    } else if(panjangModusTerbesar == arr.length) {
      return - 1;
    } else if (result[i].length == panjangModusTerbesar) {
      return result[i][0];
    }
  }
